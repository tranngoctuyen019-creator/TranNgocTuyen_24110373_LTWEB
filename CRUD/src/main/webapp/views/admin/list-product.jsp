...
<style>
	...
	.topbar a.home-link { color: var(--muted); font-size: 13.5px; text-decoration: none; border-bottom: 1px solid transparent; }
	.topbar a.home-link:hover { color: var(--ink); border-bottom-color: var(--ink); }
	.btn-add { background: var(--accent); color: #fff; font-weight: 500; border: none; border-radius: 4px; padding: 9px 18px; font-size: 14px; display: inline-block; margin-bottom: 22px; }
	.btn-add:hover { background: var(--accent-dark); color: #fff; text-decoration: none; }
	table { margin-bottom: 0 !important; background: var(--surface); }
	.table > thead > tr > th { background: transparent; color: var(--muted); border-bottom: 1px solid var(--border) !important; border-top: none !important; font-weight: 600; font-size: 12.5px; padding: 10px 12px; }
	.table > tbody > tr > td { border-top: 1px solid #eee !important; vertical-align: middle !important; padding: 12px; font-size: 14px; }
	img.thumb { border-radius: 2px; border: 1px solid var(--border); object-fit: cover; }
	.btn-sm { border-radius: 4px; font-weight: 500; padding: 5px 12px; border: 1px solid transparent; font-size: 13px; }
		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${productList}" var="p" varStatus="STT">
					<tr>
						...
						<td>
							...
							<img class="thumb" height="56" width="76" src="${imgUrl}" alt="Ảnh"
				...
				<c:if test="${empty productList}">
					<tr><td colspan="7" class="text-center empty-row">Chưa có sản phẩm nào.</td></tr>
				</c:if>
			</tbody>
		</table>
	</div>
